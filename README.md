...
Run `python3 tools/validate_mod.py` before packaging. Keep any future assets original for public distribution. `PTMP_Atmosphere.lua` uses vanilla climate channels once per in-game minute and eases into gloomy daylight, muted color, increased cloud cover, and restrained distant haze. It also tries a subtle blue-gray outdoor light tint with a gentler indoor tint; if the color API fails it logs once and keeps the float-based climate layer. Version 0.3.5 moves the sandbox labels to the Build 42.15+ JSON translation path. The tester reported **no PROJECT TERM page** while using the earlier text translation; whether the JSON change fixes that remains unverified. In a new Custom Sandbox save with this mod enabled before creation, look for Enable atmosphere, Overall atmosphere, Distant haze, Darkness, and Cold blue-gray light. Existing saves can retain old defaults. It does not make literal smoke particles or volumetric lights. Other climate mods can conflict.
