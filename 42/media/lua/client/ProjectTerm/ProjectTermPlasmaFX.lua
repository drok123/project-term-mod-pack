...
local function removeFlash()
    if activeFlash and getCell() then
        pcall(function()
            getCell():removeLamppost(activeFlash)
        end)
    end
    activeFlash = nil
    ticksLeft = 0
...
local function addPulseFlash(attacker, weapon)
    ...
    -- OnWeaponSwing fires as the attack begins. If the gun exposes ammo count,
    -- avoid flashing when completely dry.
    ...
    local okLight, light = pcall(function()
        ...
        return getCell():addLamppost(x, y, z, 0.85, 0.20, 1.00, 5)
    end)
