...
Version 0.3.2 adds an adjustable cold blue-gray global light tint. The official API documents `ClimateManager.COLOR_GLOBAL_LIGHT`, `ClimateManager:getClimateColor`, `ClimateColor:getInternalValue`/`setOverride`/`setEnableOverride`, and `ClimateColorInfo` exterior/interior colors. Their Lua binding and appearance in **42.20.4** are unverified. The mock checks intended channel calls and graceful fallback; game evidence is still required. Check a new sandbox save for the Cold blue-gray light option, an exterior at noon and night, a furnished interior, and rain; then compare with F8 and with tint set to 0. Capture `console.txt`, including any one-time tint error. The previous version 0.3.1 load does not verify 0.3.2.
