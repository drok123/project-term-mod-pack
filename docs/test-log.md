...
Version 0.3.4 removes the unsupported `next()` call and makes release idempotent on the minute timer. A Lua mock with `next = nil` passes; a fresh in-game OFF/ON log and paired screenshots remain required. The new log proves five sandbox values loaded, **not** that the PROJECT TERM options page appeared.
